...
void UART_ISP_IRQHandler(void)
{
    ...
    uint32_t u32IntSrc = UART_ISP->ISR;
    ...
    if (u32IntSrc & (UART_ISR_RDA_IF_Msk|UART_ISR_TOUT_IF_Msk)) { 
        ...
        while (((UART_ISP->FSR & UART_FSR_RX_EMPTY_Msk) == 0) && (bufhead < MAX_PKT_SIZE)) {
            uart_rcvbuf[bufhead++] = UART_ISP->RBR;
        }
    }
...
void PutString(void)
{
    ...
    for (i = 0; i < MAX_PKT_SIZE; i++) {
        ...
        while ((UART_ISP->FSR & UART_FSR_TX_FULL_Msk));

        /* UART send data */ 
        UART_ISP->THR = response_buff[i];
    }
}
...
void UART_Init()
{
    ...
    UART_ISP->FUN_SEL = UART_FUNC_SEL_UART;
    /* Set UART line configuration */
    UART_ISP->LCR = UART_WORD_LEN_8 | UART_PARITY_NONE | UART_STOP_BIT_1;
    /* Set UART Rx and RTS trigger level */
    UART_ISP->FCR = UART_FCR_RFITL_14BYTES | UART_FCR_RTS_TRI_LEV_14BYTES;
    /* Set UART baud rate */
    UART_ISP->BAUD = (UART_BAUD_MODE0 | UART_BAUD_MODE0_DIVIDER(__HIRC, 115200));
    /* Set time-out interrupt comparator */
    UART_ISP->TOR = (UART_ISP->TOR & ~UART_TOR_TOIC_Msk) | (0x40);
    ...
    UART_ISP->IER = (UART_IER_TIME_OUT_EN_Msk | UART_IER_TOUT_IEN_Msk | UART_IER_RDA_IEN_Msk);
}
